...
import { 
  ...
  Star, BookMarked, ArrowRight, X, Zap, ChevronDown
} from 'lucide-react';
...
interface StudentDashboardProps {
  userName: string;
  onLogout: () => void;
  onNavigateToPlan: (category: string | null) => void;
}
...
const CATEGORY_META: Record<string, { icon: string; color: string; desc: string }> = {
  Algebra:    { icon: '🔣', color: 'from-blue-500 to-indigo-600',    desc: 'Equations, variables & expressions'   },
  Arithmetic: { icon: '🔢', color: 'from-emerald-500 to-teal-600',   desc: 'Numbers, operations & fractions'      },
  Geometry:   { icon: '📐', color: 'from-amber-500 to-orange-600',   desc: 'Shapes, angles & measurements'        },
  Statistics: { icon: '📊', color: 'from-rose-500 to-pink-600',      desc: 'Data, probability & analysis'         },
  Functions:  { icon: '📈', color: 'from-violet-500 to-purple-600',  desc: 'Graphs, slopes & patterns'            },
  General:    { icon: '📚', color: 'from-slate-500 to-slate-600',    desc: 'Mixed topics & problem solving'       },
};
    ...
    <div className={`${lesson.bg} p-4 rounded-xl hover:shadow-md transition-all hover:scale-[1.02] cursor-pointer group`}>
      <div className="flex gap-3 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">{lesson.students} students • Avg {lesson.avg}</p>
        </div>
      </div>
      <div className="mt-3">
        ...
        <div className="bg-slate-200 h-2 rounded-full overflow-hidden">
          <div className={`${lesson.color} h-full rounded-full transition-all duration-500`} style={{ width: `${lesson.prog}%` }}/>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
      <div className="w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-[#6C4AB6] to-indigo-600 px-8 py-6 relative">
          <button onClick={onClose} className="absolute top-4 right-4 text-white/60 hover:text-white">
            <X size={20}/>
          </button>
          <div className="flex items-center gap-3 mb-1">
            <div className="bg-white/20 p-2 rounded-xl"><Brain size={20} className="text-white"/></div>
            <h2 className="text-white font-black text-xl">What do you want to focus on?</h2>
          </div>
          <p className="text-purple-200 text-sm">The AI will build your 7-day plan using topics from this area</p>
        </div>
        ...
        <div className="p-6">
          {loading ? (
            <div className="flex items-center justify-center py-10">
              <div className="w-8 h-8 border-4 border-[#6C4AB6] border-t-transparent rounded-full animate-spin"/>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3 mb-4">
              {categories.map(cat => {
                const meta = CATEGORY_META[cat] ?? { icon: '📚', color: 'from-slate-500 to-slate-600', desc: 'Study topics' };
                return (
                  <button
                    key={cat}
                    onClick={() => onSelect(cat)}
                    className="flex items-center gap-3 p-4 rounded-2xl border-2 border-slate-100 hover:border-[#6C4AB6] hover:shadow-lg transition-all group text-left"
                  >
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${meta.color} flex items-center justify-center text-xl shadow-md group-hover:scale-110 transition-transform`}>
                      {meta.icon}
                    </div>
                    <div>
                      <p className="font-black text-slate-800 text-sm">{cat}</p>
                      <p className="text-slate-400 text-[10px] leading-tight">{meta.desc}</p>
                    </div>
                  </button>
                );
              })}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-gradient-to-r from-[#6C4AB6] to-indigo-600 text-white font-black text-sm hover:shadow-lg hover:scale-[1.02] transition-all"
          >
            <Zap size={18}/> Let AI Decide Based on My Weak Areas
          </button>
          <p className="text-center text-slate-400 text-xs mt-2">AI will pick topics where you need the most improvement</p>
        </div>
      </div>
    </div>
  );
...
// ── Main Dashboard ─────────────────────────────────────────────────────────────
export default function StudentDashboard({ userName, onLogout, onNavigateToPlan }: StudentDashboardProps) {
  ...
  const dayHeaders      = [
    { letter: 'S', id: 'sun' }, { letter: 'M', id: 'mon' }, { letter: 'T', id: 'tue' },
    { letter: 'W', id: 'wed' }, { letter: 'T', id: 'thu' }, { letter: 'F', id: 'fri' },
    { letter: 'S', id: 'sat' },
  ];
  ...
  const handleOpenModal = async () => {
    ...
    if (categories.length === 0) {
      ...
      try {
        const res  = await fetch(`${API}/syllabus/categories`);
  ...
  const handleCategorySelect = (cat: string | null) => {
    ...
    onNavigateToPlan(cat);
  };

  return (
    <div className="flex h-screen overflow-hidden bg-slate-100 font-sans text-slate-900">
      <style>{`
        .glass-card { background: rgba(255,255,255,0.92); backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.5); box-shadow: 0 10px 40px rgba(0,0,0,0.06); }
      ...
      <aside className="w-72 border-r border-slate-200/80 bg-white/80 backdrop-blur-xl flex flex-col shrink-0">
        <div className="p-6 flex items-center gap-2">
          ...
          <h2 className="text-xl font-black tracking-tight bg-gradient-to-r from-[#6C4AB6] to-[#9D84D8] bg-clip-text text-transparent">NeuroFlex</h2>
        </div>

        <nav className="flex-1 px-4 space-y-1 mt-4">
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-[#6C4AB6] to-[#9D84D8] text-white shadow-lg font-bold">
            <LayoutDashboard size={20}/> Dashboard
          </button>
          {[
            { icon: <Palette size={20}/>, label: 'Themes' },
            { icon: <LineChart size={20}/>, label: 'Status' },
            { icon: <Settings size={20}/>, label: 'Settings' },
          ].map(item => (
            <button key={item.label} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-[#6C4AB6] font-semibold transition-all">
          ...
          <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-3">
            ...
            <div className="flex gap-2">
              {achievements.map((ach, i) => (
                <div key={i} className={`w-8 h-8 rounded-lg bg-gradient-to-br ${ach.color} flex items-center justify-center text-white shadow-lg`}>
                  {ach.icon}
                </div>
              ))}
            </div>
        ...
        <div className="p-4 border-t border-slate-200/80">
          <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-rose-400 hover:bg-rose-50 font-bold transition-all">
        ...
        <header className="h-16 border-b border-slate-200/80 bg-white/70 backdrop-blur-xl px-8 flex items-center justify-between shrink-0">
          <div className="flex items-center flex-1 max-w-lg">
            <div className="relative w-full">
              ...
              <input className="w-full pl-11 pr-4 py-2.5 bg-slate-100/80 rounded-2xl outline-none text-sm focus:ring-2 focus:ring-purple-200 transition-all" placeholder="Search lessons, topics, or resources..."/>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-2 border-l border-slate-200">
              ...
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#6C4AB6] to-[#9D84D8] flex items-center justify-center text-white font-black shadow-lg uppercase">
                {userName?.[0] ?? 'J'}
              </div>
            </div>
          </div>
              ...
              <div className="relative z-10 px-8 py-6 flex items-center justify-between h-full">
                <div className="max-w-md">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-black uppercase bg-white/20 px-3 py-1 rounded-full tracking-widest text-white">Welcome Back</span>
                  </div>
                ...
                <img src={studentImg} alt="student" className="absolute right-8 bottom-0 h-[92%] hidden lg:block drop-shadow-2xl pointer-events-none"/>
              </div>
            </div>
                ...
                <div className="glass-card rounded-2xl p-5 flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-lg font-black bg-gradient-to-r from-[#6C4AB6] to-[#9D84D8] bg-clip-text text-transparent">My Lessons</h2>
                    </div>
                    <button className="text-sm text-[#6C4AB6] font-bold hover:underline flex items-center gap-1">View All <ChevronRight size={14}/></button>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-5 h-72 shrink-0">
                  ...
                  <div className="glass-card rounded-2xl p-5 flex flex-col">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="p-2 bg-gradient-to-br from-amber-400 to-orange-500 rounded-xl shadow-lg"><Flame size={16} className="text-white"/></div>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <div className="relative w-20 h-20">
                        <svg className="w-20 h-20 -rotate-90">
                          ...
                          <circle cx="40" cy="40" r="36" stroke="#f59e0b" strokeWidth="8" fill="none" strokeDasharray="226" strokeDashoffset="45" strokeLinecap="round"/>
                        </svg>
                    ...
                    <div className="text-[10px] text-slate-500 mt-auto">Last activity: <span className="font-medium text-slate-700">2 hours ago</span></div>
                  </div>

                  {/* Next Up */}
                  <div className="glass-card rounded-2xl p-5 bg-gradient-to-br from-[#6C4AB6]/5 to-indigo-600/5 flex flex-col border-t-4 border-t-[#6C4AB6]/60">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="p-2 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl shadow-lg"><Brain size={16} className="text-white"/></div>
                      <h2 className="font-black text-sm">Next Up</h2>
                    </div>
                    <p className="text-sm font-bold text-slate-800 mb-1">Probability Theory</p>
                    <p className="text-xs text-slate-600 mb-3 leading-snug">Recommended based on your Statistics progress (only 20% complete)</p>
                    <div className="mb-4">
                      <div className="flex justify-between text-[10px] font-medium mb-1.5">
                        <span>Match Confidence</span><span className="text-[#6C4AB6] font-bold">87%</span>
                      </div>
                    ...
                    <div className="flex gap-2 mt-auto">
                      <button className="flex-1 bg-[#6C4AB6] text-white py-2 rounded-xl text-xs font-bold hover:shadow-lg transition-all">Start Now</button>
                      <button className="flex-1 bg-white border border-slate-300 py-2 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all">Preview</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-5 h-full">
                <div className="glass-card rounded-2xl p-5 flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="p-2 bg-gradient-to-br from-[#6C4AB6] to-[#9D84D8] rounded-xl shadow-lg"><CalendarIcon size={16} className="text-white"/></div>
                  ...
                  <div className="text-center mb-2">
                    <p className="text-xs font-bold text-[#6C4AB6]">{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</p>
                  </div>
                  <div className="grid grid-cols-7 text-[10px] text-slate-400 text-center mb-2 font-bold">
                    {dayHeaders.map(d => <span key={d.id}>{d.letter}</span>)}
                  </div>
                  <div className="grid grid-cols-7 gap-1 text-center text-xs flex-1 content-start">
                    {emptyDays.map((_, i) => <div key={`e-${i}`}/>)}
                    {days.slice(0, 28).map(d => (
                      <div key={d} className={`p-1.5 rounded-xl transition-all ${d === currentDate.getDate() ? 'bg-gradient-to-br from-[#6C4AB6] to-[#9D84D8] text-white shadow-md' : 'text-slate-600 hover:bg-slate-100 cursor-pointer'}`}>
                        {d}
                      </div>
                    ))}
                  </div>
